...
(function ($) {
    $.fn.tummoDataGridView = function (options) {
        ...
        function drawshell(vheader, vcontrols) {
            ...
            _controls = '<table><thead><tr style="height: 35px;"><th colspan="' + _lengthcolumn + '">';
            ...
            _controls += '</div><div class="group-right"><div id="pageselector"></div></div></ul></ul></div></th></tr></thead></table>';

            var _table = '<div class="table" style="height:' + config.height + 'px">' + _controls + '<table id="' + _thisID + '_table" cellspacing="0" border="0" cellpadding="0">';
            var _thead = '<thead>';
            ...
            for (var i = 0; i < _headitems.length; i++) {
                ...
                console.log(_headitems[i].width);
        ...
        function drawbody(rows) {
            ...
            let _startpage = _past * itemperrow;
            let _endrow = _currentpage * itemperrow;
            ...
            if (rowindex > 0) {
                for (var i = _startpage; i < _endrow; i++) {
                    ...
                    for (var j = 0; j < cell.length; j++) {
                        ...
                        if (_thistype == 'hours') {
                            allrows[i] += '<td' + _alig + hiden_column + ' style="width: ' + _columwidth[j] + 'px;"><div id="row_' + i + j + '" class="t-grid-cell-time" col="' + j + '" row="' + i + '"><div class="t-grid-label t-label-full" oldvalue="' + cell[j].value + '" hidenid="' + cell[j].hidenid + '" cvalue="' + cell[j].value + '" name="' + cell[j].name + '">' + cell[j].text + '</div></div></td>';
                            if (i == 0) {
                                let today = new Date();
                            ...
                            if (i == 0) {
                                let today = new Date();
                                let _cv = convertDate(today);
                                let _cv2 = convertDate2(today);
        ...
        function drawControls(controls) {
            ...
            pageselector = $('#pageselector').tummoCombobox({ onChange: f_pagechange, label: 'Chọn trang', width: 90, labelWidth: 90, url: null });
        ...
        $('#' + _thisID + '_table').find('tbody').on('click', '.t-grid-available-click', function (e) {
            ...
            var y = e.pageY;
            var offset = { x: x, y: y };
            var data = [{ id: xid, col: getcol, row: getrow, value: value, text: text, offset: offset }];
        ...
        function f_calendarChange(value, callFrom) {
            $('#' + callFrom).find('.t-grid-label').text(convertDate(value));
        ...
        function rowToJson(cellid) {
            ...
            var vl = [{ id: parent_idx, status: 'changed', row: attrs }];
            return vl;
        }

        this.getRowAtCell = function getRowAtCell(cellID) {
            let js = JSON.stringify(rowToJson(cellID));
            return js;
        }
        ...
        function getRow(rowid) {
            ...
            $('#' + rowid).children('td').each(function (e) {
                ...
                atrr = { id: getID, hidenid: getHidenID, text: getValue, name: getName, value: getValue };
                attrs.push(atrr);
            });
